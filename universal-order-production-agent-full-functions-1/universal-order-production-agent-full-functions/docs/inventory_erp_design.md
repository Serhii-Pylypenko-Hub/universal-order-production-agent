...
- Борошно base unit: `kg`; incoming `500 g` becomes `0.5 kg`.
...
All current and future web forms must follow one user-help pattern:

- show a small hint directly under/near the field that needs attention;
- distinguish an empty required field from a filled but invalid field;
- keep the user's entered value visible so it can be corrected instead of retyped;
- show a positive "entered correctly" state for fields with accepted format/range;
- show saved-state text for secret fields, because API keys/tokens are hidden again after save;
- run the same validation path from Enter, action buttons, and server-side validation responses;
- clear old hints after a successful save/reset.

This standard applies to stock, purchases, production, bot settings, AI settings, payment/delivery placeholders, and every new module added later.
...
## UX-правило для робочих вкладок

- Кожна вкладка починається з основного сценарію користувача зліва направо: спочатку дія, яку менеджер робить найчастіше, потім таблиці/перегляд результату.
- Довідникові форми створення нового продукту або матеріалу не мають стояти першими, якщо користувач зазвичай повинен вибрати існуючу позицію.
- Якщо в полі пошуку продукту або матеріалу збігу немає, система відкриває спливаюче вікно з двома варіантами: повернутись до пошуку або створити нову позицію.
- Календар є окремим модулем: перенесення/планування замовлення, блокування робочих годин, тижневе завантаження і дошка статусів не змішуються із залишками, надходженнями або техкартами.
- Для кожного наступного доопрацювання потрібно описати сценарії помилок: перше відкриття без шуму, порожнє обов'язкове поле, невалідно заповнене поле, кілька помилок одночасно, успішне збереження, системна помилка і поведінка planned/неактивних функцій.

## Обов'язковий аудит вкладок перед наступним етапом

Перед наступним функціональним доопрацюванням потрібно пройти всі web-вкладки і привести їх до єдиного операційного сценарію:

- основна дія вкладки розміщена першою;
- допоміжні дії не заважають основному потоку;
- створення нового довідника відкривається тільки після відсутності збігу або явного натискання `Створити`;
- таблиці після дії оновлюються без ручного перезавантаження сторінки;
- всі активні поля мають валідацію, підказку, фокус на першій помилці і повідомлення про успіх;
- planned-функції видимі як майбутні, але не дозволяють зберігати неповні дані;
- AI може читати/показувати/аналізувати дані вкладки, а змінювати тільки дозволені бізнес-дії з підтвердженням;
- Telegram-бот і web мають працювати з тими ж бізнес-сервісами, без дублювання логіки.

Окремо перевірити вкладки: `Огляд`, `Календар`, `Залишки`, `Надходження`, `Закупівлі`, `Виробництво`, `Техкарти`, `Бот`, `Платежі`, `AI`, `Налаштування`.
