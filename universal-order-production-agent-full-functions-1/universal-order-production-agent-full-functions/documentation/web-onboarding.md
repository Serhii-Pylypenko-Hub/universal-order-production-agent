...
Якщо статус просить заповнити підсвічені поля, `start-bot-local.bat` не зможе запустити бота. Сам start script також перевіряє `.env` перед запуском і показує список порожніх обов'язкових полів.

## Як писати боту для замовлення

Після збереження ресурсів, запуску demo workspace і запуску `start-bot-local.bat` у Telegram можна написати `/start`.
